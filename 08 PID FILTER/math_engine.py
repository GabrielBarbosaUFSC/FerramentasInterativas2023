...
class sim_on_off: 
    ...
    def plot(self, setpoint, qe, phi_noise, amp_noise, kp, ki, kd, tauf, invert, u = 0):
        amp_noise *= 10
        sp_len = len(setpoint)
        ...
        for i in range(2, sp_len):
            ...
            if u == 0: 
                ...
                yfpast = yf
                yf = (tauf)/(self.tn+tauf)*yf + (self.tn)/(self.tn+tauf)*self.y[i]
                temD = -(yf-yfpast)/self.tn*kd
                pasterror = error
