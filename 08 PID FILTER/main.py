...
def released(param):
    ...
    slider_kd.update_released()
    slider_tauf.update_released()
    #Plota os dados
    plot()
...
def get_data():
    ...
    return_list += math_engine.get_params() + [slider_kp.get(), slider_ki.get(), slider_kd.get(), slider_tauf.get()]
    return return_list
...
win = Tk()
win.title("Controlador PID - FILTRO")
...
slider_u_max= NonLinearSlider(win, 20, 60 , plot, label= 'Umax',  initial_value=5)
slider_u_min= NonLinearSlider(win, 20, 110, plot, label= 'Umin',  initial_value=-5)
...
slider_kp   = NonLinearSlider(win, 20, 340, plot, label= 'Kp', initial_value=0.5, min=-50, max=50)
slider_ki   = NonLinearSlider(win, 20, 390, plot, label= 'Ki', initial_value=0.5, min=-50, max=50)
slider_kd   = NonLinearSlider(win, 20, 440, plot, label= 'Kd', initial_value=1, min=-50, max=50)
