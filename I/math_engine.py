...
class sim_on_off: 
    ...
    def plot(self, setpoint, qe, phi_noise, amp_noise, ki, u = 0):
        amp_noise *= 10
        sp_len = len(setpoint)
        ...
        for i in range(2, sp_len):
            ...
            if u == 0: 
                temp = setpoint[i] - self.y[i]
                self.I += self.tn*ki*temp
                #print([self.I, self.tn*ki, temp])
                self.u[i] = self.saturation(self.I)
