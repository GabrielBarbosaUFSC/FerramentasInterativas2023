...
def get_data():
    ...
    return_list += math_engine.get_params() + [slider_kp.get(), slider_ki.get(), slider_kd.get()]
    return return_list
...
win = Tk()
win.title("Controlador PID - SAIDA")
...
slider_kp = NonLinearSlider(win, 20, 360, plot, label= 'Kp', initial_value=0.5, min=-50, max=50)
slider_ki = NonLinearSlider(win, 20, 410, plot, label= 'Ki', initial_value=0.5, min=-50, max=50)
slider_kd = NonLinearSlider(win, 20, 460, plot, label= 'Kd', initial_value=1, min=-50, max=50)
