...
def get_data():
    ...
    return_list += math_engine.get_params() + [slider_kp.get(), slider_bias.get()]
    return return_list
...
slider_kp = NonLinearSlider(win, 20, 400, plot, label= 'Kp', initial_value=1, min=0, max=50)
slider_bias = NonLinearSlider(win, 20, 450, plot, label= 'bias', initial_value=0, min=-50, max=50)

#Cria um botão para poder gerar uma forma de onda 
Button(win, text= "Sinal", command= wave_create.create_win).place(x = 20, y = 500, height= 30, width= 150)
