...
def plot():
    ...
    y_graph.set_data([y_sp, math_engine.y, math_engine.q]+data_plot_y, ["setpoint", "y", "q"]+data_plot_ylabel)
    
#Faz com que os sliders voltam para a posição zero depois que vc solta o cursor
def released(param):
    slider_a.update_released()
...
def get_data():
    ...
    return_list += math_engine.get_params() + [slider_kp.get(), slider_ki.get(), slider_ks.get()]
    return return_list


#Faz as configurações iniciais da janela
win = Tk()
win.title("Controlador PI + AWP - Discreto")
...
#Cria os gráficos 
y_graph = Graph(win, 290, 20, math_engine.get_time, "Amostras", "Saída")
u_graph = Graph(win, 290, 320, math_engine.get_time, "Amostras", "Controle")  
...
slider_u_max= NonLinearSlider(win, 20, 10 , plot, label= 'Umax',  initial_value=3)
slider_u_min= NonLinearSlider(win, 20, 60, plot, label= 'Umin',  initial_value=-3)
slider_ke   = NonLinearSlider(win, 20, 110, plot, label= 'Ke',    initial_value=1)
slider_kq   = NonLinearSlider(win, 20, 160, plot, label= 'Kq',    initial_value=1)
slider_l    = NonLinearSlider(win, 20, 210, plot, label= 'L',     initial_value=0, min=0, max=25, IsInt=True)

slider_a  = NonLinearSlider(win, 20, 310 , plot, label= 'a', initial_value=0.99, min= 0.85, max=0.999) 
slider_kp = NonLinearSlider(win, 20, 360, plot, label= 'Kp', initial_value=7, min=-15, max=15)
slider_ki = NonLinearSlider(win, 20, 410, plot, label= 'Ki', initial_value=1, min=-15, max=15)
slider_ks = NonLinearSlider(win, 20, 460, plot, label= 'Ks', initial_value=0, min=-15, max=15)
slider_ks.hide()

var1 = IntVar()
c1 = Checkbutton(win, text='Anti Windup',variable=var1, onvalue=1, offvalue=0, command=plot)
c1.place(x=20, y=460)
