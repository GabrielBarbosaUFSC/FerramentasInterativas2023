...
class Save:
    def __init__(self, win, pos_x, pos_y, number, t):
        self.t = t
        self.datas = [[], [], [], []] # [y, u, sp, q]
        self.condicions = [] # [tau, u_max, u_min, ke, kq, l, tol]
        self.number = number
        self.in_use = False
        self.win = win
        self.posx = pos_x
    ...
    def set(self, params):
        ...
        self.condicions = params[4:11]
        self.label_var.set(f'#{self.number}: {params[11]}')
    ...
    def show_wave_form(self):
        ...
        values_min = [i - self.condicions[6] for i in self.datas[2]]
        values_max = [i + self.condicions[6] for i in self.datas[2]]
        
        self.plot_area.plot(self.time, values_max, label = "y max")
        self.plot_area.plot(self.time, values_min, label = "y min")
        ...
        labels = ["\u03c4", "u_max", "u_min", "Ke", "Kq", "L", "Banda"]
        text = ""
        for i in range(7):
